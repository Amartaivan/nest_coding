...
vector<int> quicksort(vector<int> array){
    ...
    if (array.size() > 1){
        ...
        auto sort_array = 
            [](vector<int>& array, int& finished_threads) { 
                array = quicksort(array);
                return ++finished_threads;
            };

        thread less_sort(sort_array, ref(less), ref(finished_threads)), greater_sort(sort_array, ref(greater), ref(finished_threads));
        less_sort.join();
        greater_sort.join();
